...
class ImgFeatureExtractor:
    def __init__(
        ...
        self.dimResize = (
            int(imgDim[0] / self.resizeFac),
            int(imgDim[1] / self.resizeFac),
        )
    ...
    def color_space_cvt(self, imgURL):
        if self.color_space == "Gray":
            ...
            imgTmp = cv2.cvtColor(
                imgTmp, cv2.COLOR_BGR2HSV
            )  # right way to convert to hsv
        else:
    ...
    def resizeAndConcatenate(self, coordFileName, limit, use_high_pass_filter="False"):
        ...
        if self.nCh > 1:
            inputs = np.zeros(
                [nSamples, dimResize[0], dimResize[1] * 3, self.nCh], dtype=np.uint8
            )
        else:
            inputs = np.zeros(
                [nSamples, dimResize[0], dimResize[1] * 3, 1], dtype=np.uint8
            )
        ...
        for samp in range(0, nSamples):
            for cam in range(1, 4):
                ...
                imgURL = (
                    ...
                    + str(cam)
                    + "/"
                    + "{:0>1}".format(epi_scen[0])
                    + ".png"
                )
                imgTmp = self.color_space_cvt(
                    imgURL
                )  # convert for the right color space
                # cv2.imshow("Resized image", imgTmp) #plot debug
                # cv2.waitKey(0)
                # cv2.destroyAllWindows()
                imgRes = cv2.resize(
                    imgTmp, (dimResize[1], dimResize[0]), interpolation=cv2.INTER_AREA
                )
                ...
                if self.nCh == 1:
                    if use_high_pass_filter == True:
                        ...
                        inputs[
                            samp, :, dimResize[1] * (cam - 1) : dimResize[1] * cam, 0
                        ] = imgResFilt
                    else:
                        inputs[
                            samp, :, dimResize[1] * (cam - 1) : dimResize[1] * cam, 0
                    ...
                    inputs[
                        samp, :, dimResize[1] * (cam - 1) : dimResize[1] * cam, :
                    ] = imgRes
        ...
        np.savez(
            self.inputDataDir + "img_input_train_" + str(self.resizeFac) + ".npz",
            inputs=input_train,
        )
        np.savez(
            self.inputDataDir + "img_input_validation_" + str(self.resizeFac) + ".npz",
