...
def main():
    ...
    if len(argv) < 5:
        print(
            "Please, use the following command: blender your_scenario.blend -P blender_animation.py Your_InSite_Simulation_Folder"
        )
...
def animateVehiclesBlender(vPosition, frame_num, frame_step):
    ...
    for x in range(0, len(C.scene.objects)):
        obj_name = C.scene.objects[x].name
        if (
            obj_name.startswith("flow")
            or obj_name.startswith("dflow")
            or obj_name.startswith("ped")
        ):  # Add to list
    ...
    for vehicles in vPosition.items():
        if D.objects.get(vehicles[0]) is not None:  # Existe, code to move
            ...
            if float(vehicles[1]["height"]) == 1.59:  # Car
                bpy.ops.wm.append(
                    ...
                    + "//vehicles.blend/Object/",
                    filepath="vehicles.blend",
                    filename="Car",
                )
                veh = D.objects["Car"]
        ...
        veh.location.xyz = (
            float(vehicles[1]["xinsite"]),
            float(vehicles[1]["yinsite"]),
            float(vehicles[1]["z3"]),
        )  # float(vehicles[1]['height'])/2 # X,Y,Z
        veh.keyframe_insert(data_path="hide_render", index=-1)
        veh.keyframe_insert(data_path="hide", index=-1)
